...
from plotter.plot_base import Plotter
...
class PnlDecay(Plotter): 
    def __init__(self, feed='feed_kraken_tob_5', start_ts=None, end_ts=None, lookback_minutes=15, value_mode='price', *args, **kwargs):
        ...
        # value_mode controls whether we plot raw price PnL or normalized returns
        # accepted values: 'price' (default) or 'return'
        self.value_mode = (value_mode or 'price').lower()
        if self.value_mode not in ('price', 'return', 'returns'):
            self.value_mode = 'price'

        self.usd_conversion_map = USD_CONVERSION_MAP

        self.decay_df = None
        self.plots = defaultdict(list)
    
    def _get_neighbor_dfs(self, ts, feed=None, instrument=None):
        fmt = '%Y-%m-%dT%H:%M:%S.%fZ'
        ts_start = ts - pd.Timedelta(minutes=self.lookback_minutes)
        ts_end   = ts + pd.Timedelta(minutes=self.lookback_minutes)

        df_before = self.fetcher.fetch_qdb(
            self.feed,
            ts_start.strftime(fmt),
            ts.strftime(fmt),
            columns='ts_server,ts_exch,ask_px_0,bid_px_0',
            instrument=instrument
        )

        df_after = self.fetcher.fetch_qdb(
            self.feed,
            ts.strftime(fmt),
            ts_end.strftime(fmt),
            columns='ts_server,ts_exch,ask_px_0,bid_px_0',
        ...
        # Get USD conversion instrument from map
        usd_instrument = self.usd_conversion_map.get(instrument)
        df_before_usd = pd.DataFrame()
        df_after_usd = pd.DataFrame()
        
        if usd_instrument:
            df_before_usd = self.fetcher.fetch_qdb(
                self.feed,
                ts_start.strftime(fmt),
                ts.strftime(fmt),
                columns='ts_server,ts_exch,ask_px_0,bid_px_0',
            ...
            df_after_usd = self.fetcher.fetch_qdb(
                self.feed,
                ts.strftime(fmt),
                ts_end.strftime(fmt),
                columns='ts_server,ts_exch,ask_px_0,bid_px_0',
        ...
        return df_before, df_after, df_before_usd, df_after_usd

    def _preprocess(self, resample=None):
        df_deals = self.fetcher.fetch_qdb('deals', self.start_ts, self.end_ts)
        # print(df_deals) #TODO: Remove this
        
        # Check if DataFrame is empty
        if df_deals.empty:
            print("No deals data found for the specified time range. Cannot generate PnL decay plots.")
            self.decay_df = pd.DataFrame()
            return
        
        df_deals['time'] = pd.to_datetime(df_deals['time'])

        # Apply and expand tuple into four new columns
        df_deals[['before', 'after', 'before_usd', 'after_usd']] = (
            df_deals.apply(
                lambda row: self._get_neighbor_dfs(
                    ts=row['time'],
                    feed=self.feed,
        ...
        # Apply optional resampling while keeping values as DataFrames
        rule = None
        if isinstance(resample, int) and resample > 0:
            rule = f"{resample}ms"
        elif isinstance(resample, str) and resample.strip():
            rule = resample.strip()

        def _resample_df(df, rule):
            if not isinstance(df, pd.DataFrame) or df.empty or rule is None:
                return df
            if 'ts_server' not in df.columns:
                return df
            cols = [c for c in ['ask_px_0', 'bid_px_0'] if c in df.columns]
            if not cols:
                return df
            s = df.copy()
            ts = pd.to_datetime(s['ts_server'], utc=True, errors='coerce')
            s = s.assign(_ts=ts).dropna(subset=['_ts']).set_index('_ts')
            try:
                s_res = s[cols].resample(rule).mean().dropna(how='all').reset_index().rename(columns={'_ts': 'ts_server'})
                s_res['ts_server'] = s_res['ts_server'].dt.tz_convert('UTC').dt.strftime('%Y-%m-%dT%H:%M:%S.%fZ')
                return s_res
            except Exception:
                return df

        if rule is not None:
            for col in ['before', 'after', 'before_usd', 'after_usd']:
                if col in df_deals.columns:
                    df_deals[col] = df_deals[col].apply(lambda x: _resample_df(x, rule))

        self.decay_df = df_deals
